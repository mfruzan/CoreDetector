...
public class MAFProcessor {
    ...
    public void generateMSA_old(String srcDir, String maffFiles, String fafFile, String mafFile, String nameFile){
        // Developed 9/1/2020 with Nickie Bakaj
    ...
    public void generateMSA(String srcDir, String maffFiles, String fafFile, String mafFile, String nameFile){
        ...
        // This method generated fa alignment file (this needs enough memory, to attach all the small sequnces relarated to one specie in one big sequnce)
        // the second output is maf file 
        //whenever said query means a sequence from the first genome, all the other genomes are subjects
        FileWriter fafWriter = null;
        FileWriter mafWriter = null;
        BufferedReader nameReader = null;
        ...
        try {
            fafWriter = new FileWriter(fafFile);         
            mafWriter = new FileWriter(mafFile);
            System.out.println("Processing Maf Files at : " + srcDir );
            String[] files = maffFiles.split(",");
            ...
            // the following 2 list have size of mafList + 1, because for n genome we will have n-1 maf files
            List<StringBuffer> buffList = new ArrayList(); // this List needs enough memory 
            List<StringBuffer> partialBuffList = new ArrayList();// each Buffer is used for one genome, multiple sequence alignment is stored here, first element stores query sequence from first genome, then all the subjects from last genome to the second genome
            // add first element (used for query genome or first genome)
            buffList.add(new StringBuffer());
            partialBuffList.add(new StringBuffer());
            // we add maf files from last to first one into mafList
            for(int i=0; i<files.length; i++){
                // this part can be memory intensive, we need to have space for all maf files in the memory
                MAFProcessor maf = new MAFProcessor(srcDir + "/" +files[i]);
                maf.loadBinary();//BlastHSP like entries
                mafList.add(maf);
                buffList.add(new StringBuffer());
                partialBuffList.add(new StringBuffer());
            }
            ...
            nameReader.close();
            if (names.size() != files.length+1)
                throw new Exception("Number of names in " + nameFile +" does not match number of MAF files");
            //co ordinates of subjects are absulute but query is always relative to previous step for example if query is something like : qcontig!20!15!2!4!1!2
            //the absolute coordinates of query is calculated recursively at each step co-ordinates are added to start co-ordinate of previous step: starting from end 1+2-1=2 and 2+2-1=3, then 2 and 3 are going to be added to the start of step before: 2+20-1=21 3+20-1=22  so absulute values are 21 until 22 of the qcontig
            int total_len = 0;
            for(BlastHSP hsp : mafList.get(0).all_hsp){
                ...
                if (hsp.q_contig.equals("ST00Arg_chr_1!2300082!2384753!21455!49311!9796!27857!6720!18062!1!11343!1!11343!1317!11343!1!10023") && hsp.q_start==1 && hsp.q_end==4041)
                    hsp.q_start=1;
                //reset partialBuffList
                for(StringBuffer bf : partialBuffList)
                    bf.delete(0, bf.length());
                //for the last maf file
        if (hsp.s_contig.equals("MUXC01000133.1"))
            log = true;
                //first write query part in positive strand to original contig
                if(hsp.q_pos_strand)
                  partialBuffList.get(0).append(hsp.q_alignment_block);
                else
                  partialBuffList.get(0).append(Helper.getRC(hsp.q_alignment_block));
                if(log){
                    System.out.println("start logging");
                    System.out.println(hsp.q_pos_strand?"+":"-" + partialBuffList.get(0));
                }
                // a MAF entry will be created
                BlastHSP mafEntry = new BlastHSP();
                mafEntry.mafLines = new ArrayList();
                mafEntry.mafLines.add(new MAFSLine()); // this is only a place holder for query contig, its values will be calculated (using 4 next variables) and replaced after following loop
                ...
                //Following 2 variables are used inside alignSubjectPart3 to extract part of query and subject alignment blocks related to 'hsp' variable, both query and subject blocks have to be trimmed
                //These 2 indexes are calculated based on query contig,  both are 1-based and inclusive indexes, gets updated at the end of each maf file loop (next loop)
                // For the first maf whole query and subject are extracted so initialize them to the start and end of query  
                int offset_start = 1;
                int offset_end = hsp.q_end - hsp.q_start + 1;                                 
                ...
                // we creates a list of StringBuffer and for each extracted subject we insert string into it
                try{
                    // number of subjects equals number of maf files, starting from last maf file to the first we extract subject part and realign with previous extracted sequences
                    for(int i = 0; i<mafList.size(); i++){
                        MAFProcessor maf = mafList.get(i);
                        // we update searching_contig searching_start, searching_end  at the end of this loop
                        // first find HSP with relevant query in the maf file
                        BlastHSP aHSP = maf.searchQuery(searching_contig, searching_start, searching_end);
                      ...
                      aHSP.alignSubjectPart3(offset_start, offset_end, i+1, partialBuffList, log);

                        if(partialBuffList.get(i+1).length()==0)
                            throw new Exception("Something goes wrong!"); // this will cause whole alignment for current query will be discarded

                        // now we have information to write MAFSLine for this subject part, s_start and s_end have been updated in aHSP.alignSubjectPart3
                        // This constructor read as BlastHSP format
                        MAFSLine line = new MAFSLine(aHSP.s_contig, aHSP.s_start, aHSP.s_end, aHSP.q_pos_strand, aHSP.s_contig_size, "");
                        mafEntry.mafLines.add(line);
                        //update search_start, search_end, search_contig. offset_start, offset_end
                        if (aHSP.q_contig.indexOf('!')>0){
                            // if there is one level above; we dont enter this 'if' condition for the last MAF file
                    ...
                    // now we have correct absolute coordinates of query contig, we add it query to MAFLines, query sequence always has + strandness
                    MAFSLine line = new MAFSLine(absolute_qcontig_name, absolute_qcontig_start, absolute_qcontig_end, true, absolute_qcontig_size, "");
                    mafEntry.mafLines.set(0, line);
            ...
            System.out.println("Length of Alignment " + buffList.get(0).length());
            System.out.println("Total Query Length " + total_len);
            System.out.println("Done!");
    ...
    private BlastHSP getNext_old(){
        ...
        // We change orientation of query and subject to be like Blast
        ...
        try{
            ...
            while(line!= null){
                ...
                if(line.matches("a\\sscore=.+")){
                    ...
                    String s_alignment_block = parts[6];
                    hsp.s_contig_size = Integer.parseInt(parts[5]);
                    if(parts[4].equals("+")){
                        // positive orientation
                        hsp.s_pos_strand = true;
                        hsp.s_start = Integer.parseInt(parts[2])+1; // convert zero based into 1 based
                        hsp.s_end = hsp.s_start + hsp.s_len -1;
                    }else{
                        // negative orientation (start>end)
                        hsp.s_pos_strand = false;
                        
                        hsp.s_end = hsp.s_contig_size - Integer.parseInt(parts[2]);
                    ...
                    String q_alignment_block = parts[6];
                    if(parts[4].equals("+")){
                        // positive orientation  
                        hsp.q_pos_strand = true;
                        hsp.q_start = Integer.parseInt(parts[2])+1; // convert zero based into 1 based
                        hsp.q_end = hsp.q_start + hsp.q_len -1;
                    }else{
                        // negative orientation    
                        hsp.q_pos_strand = false;
                        hsp.q_end = hsp.q_contig_size - Integer.parseInt(parts[2]);
                        hsp.q_start = hsp.q_end - hsp.q_len + 1;
                    }
                    // now work out homolg part of query region that is actually subject region
                    if(hsp.s_pos_strand ^ hsp.q_pos_strand){
                        // reverse orientation (+/- OR -/+)
                        hsp.orientation = 0;
                        hsp.q_alignment_block = Helper.getRC(q_alignment_block) ;
                        hsp.s_alignment_block = Helper.getRC(s_alignment_block) ;
                        //hsp.q_alignment_block = q_alignment_block ;
                        //hsp.s_alignment_block = s_alignment_block ;

                    }else{
                        // the same orientation (+/+ OR -/-; However -/- should never happens)
                        hsp.orientation = 1;
                        hsp.q_alignment_block = q_alignment_block;
                        hsp.s_alignment_block = s_alignment_block;
                    }
        ...
        try{
            ...
            while(hsp != null){
                ...
                if (lenbp != null)
                    ...
                    lens.put(size, lbp);
                //remove the first line from the maf
